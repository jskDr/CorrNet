...
class CorrNet(object):
    ...
    def init(self, numpy_rng, theano_rng=None, l_rate=0.01, optimization="sgd",
        ...
        if not tied:
            ...
            self.optimizer.register_variable(
                "W_left_prime", n_hidden, n_visible_left)
            self.W_right_prime = self.Initializer.fan_based_sigmoid(
                "W_right_prime", W_right_prime, n_hidden, n_visible_right)
            self.optimizer.register_variable(
                "W_right_prime", n_hidden, n_visible_right)
        else:
        ...
        if tied:
            self.params = [self.W_left, self.W_right,  self.b,
                           self.b_prime_left, self.b_prime_right]
            self.param_names = ["W_left", "W_right",
                                "b", "b_prime_left", "b_prime_right"]
        else:
            self.params = [self.W_left, self.W_right,  self.b, self.b_prime_left,
                           self.b_prime_right, self.W_left_prime, self.W_right_prime]
            self.param_names = ["W_left", "W_right", "b", "b_prime_left",
                                "b_prime_right", "W_left_prime", "W_right_prime"]

        self.proj_from_left = theano.function(
            [self.x_left], self.project_from_left())
        self.proj_from_right = theano.function(
            [self.x_right], self.project_from_right())
        self.recon_from_left = theano.function(
            [self.x_left], self.reconstruct_from_left())
        self.recon_from_right = theano.function(
            [self.x_right], self.reconstruct_from_right())
    ...
    def train_common(self, mtype="1111"):
        ...
        L1 = loss(z1_left, self.x_left, self.loss_fn) + \
            loss(z1_right, self.x_right, self.loss_fn)
        ...
        L2 = loss(z2_left, self.x_left, self.loss_fn) + \
            loss(z2_right, self.x_right, self.loss_fn)

        y3_pre = T.dot(self.x_left, self.W_left) + \
            T.dot(self.x_right, self.W_right) + self.b
        ...
        L3 = loss(z3_left, self.x_left, self.loss_fn) + \
            loss(z3_right, self.x_right, self.loss_fn)
        ...
        L5 = loss(lz4_right, self.x_right, self.loss_fn) + \
            loss(rz4_left, self.x_left, self.loss_fn)
    ...
    def train_left(self):
        ...
        if self.tied:
            ...
            curr_params = [self.W_left, self.b,
                           self.b_prime_left, self.W_left_prime]
    ...
    def train_right(self):
        ...
        if self.tied:
            ...
            curr_params = [self.W_right, self.b,
                           self.b_prime_right, self.W_right_prime]
            curr_param_names = ["W_right", "b",
                                "b_prime_right", "W_right_prime"]
    ...
    def load(self, folder, input_left=None, input_right=None):
        ...
        self.init(plist["numpy_rng"], theano_rng=plist["theano_rng"], l_rate=plist["l_rate"],
                  ...
                  n_visible_left=plist["n_visible_left"], n_visible_right=plist[
                      "n_visible_right"],
                  n_hidden=plist["n_hidden"], lamda=plist[
                      "lamda"], W_left=folder + "W_left",
                  ...
                  hidden_activation=plist["hidden_activation"], output_activation=plist[
                      "output_activation"],
...
def trainCorrNet(src_folder, tgt_folder, batch_size=20, training_epochs=40,
    ...
    model = CorrNet()
    model.init(numpy_rng=rng, theano_rng=theano_rng, l_rate=l_rate, optimization=optimization, tied=tied, n_visible_left=n_visible_left, n_visible_right=n_visible_right, n_hidden=n_hidden, lamda=lamda, W_left=W_left, W_right=W_right, b=b, W_left_prime=W_left_prime,
               W_right_prime=W_right_prime, b_prime_left=b_prime_left, b_prime_right=b_prime_right, input_left=x_left, input_right=x_right, hidden_activation=hidden_activation, output_activation=output_activation, loss_fn=loss_fn, op_folder=tgt_folder)
    # model.load(tgt_folder,x_left,x_right)
    start_time = time.clock()
    train_set_x_left = theano.shared(numpy.asarray(numpy.zeros(
        (1000, n_visible_left)), dtype=theano.config.floatX), borrow=True)
    train_set_x_right = theano.shared(numpy.asarray(numpy.zeros(
        (1000, n_visible_right)), dtype=theano.config.floatX), borrow=True)

    common_cost, common_updates = model.train_common("1111")
    mtrain_common = theano.function([index], common_cost, updates=common_updates, givens=[(x_left, train_set_x_left[
                                    index * batch_size:(index + 1) * batch_size]), (x_right, train_set_x_right[index * batch_size:(index + 1) * batch_size])])

    left_cost, left_updates = model.train_left()
    mtrain_left = theano.function([index], left_cost, updates=left_updates, givens=[
                                  (x_left, train_set_x_left[index * batch_size:(index + 1) * batch_size])])

    right_cost, right_updates = model.train_right()
    mtrain_right = theano.function([index], right_cost, updates=right_updates, givens=[
    ...
    for epoch in range(training_epochs):
        ...
        for line in ipfile:
            ...
            if(next[0] == "xy"):
                if(next[1] == "dense"):
                    denseTheanoloader(next[2] + "_left",
                                      train_set_x_left, "float32")
                    denseTheanoloader(next[2] + "_right",
                                      train_set_x_right, "float32")
                else:
                    sparseTheanoloader(
